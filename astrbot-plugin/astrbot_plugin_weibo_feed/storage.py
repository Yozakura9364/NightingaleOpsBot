...
class WeiboFeedStore:
    def __init__(self, data_dir: Path):
        self.data_dir = data_dir
        self.data_dir.mkdir(parents=True, exist_ok=True)
        self.db_path = self.data_dir / "weibo_feed.sqlite3"
    ...
    def _init_db(self) -> None:
        with self._lock, self._connect() as connection:
            ...
            connection.execute(
                "CREATE INDEX IF NOT EXISTS idx_weibo_subscriptions_origin ON subscriptions(target_origin)"
            )
        ...
        with self._lock, self._connect() as connection:
            ...
            connection.execute(
                ...
                INSERT INTO subscriptions (
                    uid, target_origin, target_kind, created_by, created_at,
                    enabled, last_seen_id, last_seen_link
                )
                VALUES (?, ?, ?, ?, ?, 1, ?, ?)
                """,
                (uid, target_origin, target_kind, created_by, now, last_seen_id, last_seen_link),
            )
            row = connection.execute(
                "SELECT * FROM subscriptions WHERE uid = ? AND target_origin = ?",
                (uid, target_origin),
