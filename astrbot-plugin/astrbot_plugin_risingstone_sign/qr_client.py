...
class RisingstoneQrClient:
    ...
    async def start_session(self, *, ttl_ms: int) -> dict:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                f"{self.endpoint}/risingstone/qr/start",
    ...
    async def download_image(self, session_id: str, output_path: Path) -> None:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                f"{self.endpoint}/risingstone/qr/{session_id}/image",
                headers=headers,
            ) as response:
    ...
    async def status(self, session_id: str) -> dict:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                f"{self.endpoint}/risingstone/qr/{session_id}/status",
                headers=self._headers(),
            ) as response:
                return await self._read_json(response)
    ...
    async def cancel(self, session_id: str) -> None:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.delete(
                f"{self.endpoint}/risingstone/qr/{session_id}",
                headers=self._headers(),
            ) as response:
                if response.status >= 400:
                    await response.read()
